...
def calculate_accrual_basis_corp():
    timestr = time.strftime("%Y%m%d-%H%M%S")

    Taxable_Income = float(raw_input(yellow("Enter the corporation's taxable income: ")).replace(',',''))
    ...
    deduction_limit = (0.1 * taxable income)
    ...
    if elect_question_preceding_year == "n":
        string_instant_deductible_cc = "CURRENT YEAR Instant-Deductible (CORPORATE) through Charitable Contribution: " + deduction_limit
                                        + '\n'
                                        + "EXCESS Charitable Contribution to be deducted annually: "
                                        + excess_cc_annually
